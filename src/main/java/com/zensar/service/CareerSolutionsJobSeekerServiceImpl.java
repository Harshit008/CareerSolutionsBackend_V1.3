...
public class CareerSolutionsJobSeekerServiceImpl implements CareerSoltionsJobSeekerService {
	...
	public JobSeeker registerJobSeeker(JobSeeker jobSeeker) throws GlobalExceptionHandler {
		...
		mailService.sendMail(new NotificationEmail("Please Activate your account", jobSeeker.getEmail(),
				"Thank you for signing up to Career Solutions, "
						+ "please click on the below url to activate your account : "
						+ "http://http://careersolutionsbackendv1-env.eba-umxntpch.ap-south-1.elasticbeanstalk.com/myapp/verifyJobSeeker/" + token));
		if (res)
			return jobSeeker;
		else
			return null;
	...
	@Transactional
	private void fetchJobSeekerAndEnable(JobSeekerVerificationToken jobSeekerVerificationToken)
	...
	public JobSeekerAuthenticationResponse jobSeekerlogin(JobSeeker jobSeeker) {
		...
		Authentication authenticate = authenticationManager.authenticate(
				new UsernamePasswordAuthenticationToken(jobSeeker.getUsername(), jobSeeker.getPassword()));
		System.out.println(authenticate);
		JobSeekerDetails jobSeekerDetails = (JobSeekerDetails) this.jobSeekerDetailService
				.loadUserByUsername(jobSeeker.getUsername());
	...
	@Override
	public void insertJobSeeker(JobSeeker jobSeekerByUsername) {
			jobSeekerRepository.save(jobSeekerByUsername);
	}
